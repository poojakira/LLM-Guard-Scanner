...
def check(title, content, source="user", tool_args=None, caller_id=None):
    ...
    tag = "[BLOCKED]" if sev in ("HIGH", "CRITICAL") else "[MEDIUM]" if sev == "MEDIUM" else "[CLEAN]"
    print(f"\n{SEP}")
    print(f"TEST   : {title}")
...
def main():
    ...
    assert r1.highest_severity in ("HIGH", "CRITICAL"), f"FAIL: expected HIGH/CRITICAL got {r1.highest_severity}"
    ...
    r5 = check(
        "Clean user message - should pass",
